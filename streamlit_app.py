...
with tab_local:
    ...
    # ── CLOUD MODE: folder upload via Streamlit native uploader + webkitdirectory ─
    if not TKINTER_OK:
        st.markdown("#### Select your PDFs folder")

        import streamlit.components.v1 as components

        # Inject webkitdirectory into Streamlit's native file uploader input.
        # This makes the "Browse files" button open a FOLDER picker instead of
        # a file picker — and drag-dropping a folder also works natively.
        # Files go through Streamlit's own optimised chunked uploader (fast).
        inject_js = """
<script>
(function() {
  function patchUploader() {
    // Find all hidden file inputs inside Streamlit's uploader widget
    ...
    inputs.forEach(inp => {
      if (!inp.hasAttribute('webkitdirectory')) {
        ...
        inp.removeAttribute('accept');   // allow all files inside folder
      }
    });
  }
  // Run immediately and keep re-patching (Streamlit re-renders widgets)
  patchUploader();
  const obs = new MutationObserver(patchUploader);
  obs.observe(window.parent.document.body, { childList: true, subtree: true });
})();
</script>
"""
        components.html(inject_js, height=0, scrolling=False)

        uploaded_files = st.file_uploader(
            "📂 Drop a folder here — or click **Browse files** to select a folder",
            type=None,          # accept everything; JS removes the filter
        ...
        if pdf_files:
            ...
            with st.expander(f"📂 Preview {len(pdf_files)} PDFs"):
                c1, c2 = st.columns(2)
                with c1:
            ...
            st.info("👆 Drop a folder or click **Browse files** above to select a folder of PDFs.")
