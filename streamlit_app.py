...
#     """Read OAuth credentials from .streamlit/secrets.toml"""
#     try:
...
#             "redirect_uris": [s.get("redirect_uri", "http://localhost:8501")],
...
#     return Flow.from_client_config(cfg, scopes=SCOPES, redirect_uri=redirect)
...
#         secrets_ok = bool(_go.get("client_id") and _go.get("client_secret"))
#     except Exception:
#         secrets_ok = False
#     if not secrets_ok:
...
# 5. Go to **APIs & Services → OAuth consent screen** → External → fill App name & email
# 6. Go to **APIs & Services → Credentials → Create Credentials → OAuth 2.0 Client ID**
# 7. Application type: **Web application**
# 8. Under *Authorized redirect URIs*, add: `http://localhost:8501`
# 9. Copy **Client ID** and **Client Secret** into `.streamlit/secrets.toml` as shown above
# 10. Restart the Streamlit app
#             """)
#     else:
#         # ── Not logged in ──────────────────────────────────────────────────────
...
# Scopes match NextAuth: "openid email profile drive.readonly" + offline access
SCOPES = [
    "openid",
    "https://www.googleapis.com/auth/userinfo.email",
    "https://www.googleapis.com/auth/userinfo.profile",
...
def _get_client_config():
    """
    Read OAuth credentials — mirrors NextAuth's process.env approach.
...
def _build_flow():
    ...
    # Disable PKCE on the underlying OAuth2 session so no code_challenge
    # is ever added to the auth URL — prevents "Missing code verifier" error
...
def _get_auth_url():
    ...
    # Strip code_challenge params if they snuck in anyway
...
with tab_drive:
    ...
    # Check secrets are configured
    import os
    # Check env vars first (like NextAuth process.env), fallback to secrets.toml
    client_id     = os.environ.get("GOOGLE_CLIENT_ID", "")
    client_secret = os.environ.get("GOOGLE_CLIENT_SECRET", "")
    if not client_id or not client_secret:
        try:
    ...
    if not secrets_ok:
        ...
        st.code("""
# .env file  (same as NextAuth — uses same variable names)
